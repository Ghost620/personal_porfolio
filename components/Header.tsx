  ...
  return (
    <header className='sticky top-0 p-5 flex items-start justify-between max-w-7xl mx-auto z-20 xl:items-center'>
      <motion.div initial={{ x: -500, opacity: 0, scale: 0.5 }} animate={{ x: 0, opacity: 1, scale: 1 }} transition={{ duration:1.5 }} className='flex flex-row items-center'>
        ...
        { socials.map((social) => isDark ? (
          <SocialIcon key={social._id} url={social.url} fgColor='silver' bgColor='transparent' />
        ) : (
          <SocialIcon key={social._id} url={social.url} fgColor='green' bgColor='transparent' />
        )
        ) }
      ...
      <motion.div initial={{ x: 500, opacity: 0, scale: 0.5 }} animate={{ x: 0, opacity: 1, scale: 1 }} transition={{ duration:1.5 }} className='flex flex-row items-center text-gray-300 cursor-pointer'>
        ...
        { isDark ? <SunIcon onClick={toggleDark} className='h-8' /> : <MoonIcon onClick={toggleDark} className='h-8 text-green-700' /> }

        <Link href="#contactMe">
          { isDark? <SocialIcon className='cursor-pointer color-green' network='email' fgColor='silver' bgColor='transparent' href="#contactMe" type='link' /> : <SocialIcon className='cursor-pointer color-green' network='email' fgColor='green' bgColor='transparent' href="#contactMe" type='link' /> }
